...
async function startup(addonData, reason) {
  ...
  webExtension.startup().then(api => {
    ...
    browser.runtime.onMessage.addListener((msg, sender, sendReply) => {
      let {header, data} = msg;
      if (header == 'show-notification') showNotification();
      if (header == 'hide-notification') hideNotification();
      if (header == 'autohide-notification') autohideNotification();
      if (header == 'form-opened ') autohideNotification();
      if (header == 'self-uninstall') studyUtils.uninstall();
    });
  });
...
function showNotification(){
  ...
  let paButton = win.document.getElementById('web-health-study_shield_mozilla_com-page-action');
  if (!paButton)
    return;

  paButton.click();

  let paPanel = win.document.getElementById('web-health-study_shield_mozilla_com-panel');
  paPanel.setAttribute('noautohide','true');
  recentPaPanel = Cu.getWeakReference(paPanel);
}

function hideNotification(){
  // check if it's the initally opened panel
  if (recentPaPanel.get() && recentPaPanel.get().hidePopup){
    recentPaPanel.get().hidePopup();
  }
...
function autohideNotification(){ // revert then panel back to its noautohide = false state
  if (recentPaPanel.get())
    recentPaPanel.get().setAttribute('noautohide', 'false');
}
